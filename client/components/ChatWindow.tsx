...
export const ChatWindow: React.FC = () => {
  const { activeConversationId, messages, conversations, addMessage, updateLastMessage } = useChatStore();
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if (!inputText.trim() || !activeConversationId || !me) return;
    ...
    updateLastMessage(activeConversationId, newMessage.content, newMessage.timestamp);
  ...
  if (!activeConversationId) {
    return (
      <div className="flex-1 h-full flex flex-col items-center justify-center bg-slate-50 p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-800">Select a chat to start messaging</h2>
        <p className="text-slate-500 mt-2 max-w-xs">Private, secure, and blazing fast communications await you.</p>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col h-full bg-[#E7EBF3]">
      ...
      <header className="bg-white px-6 py-3 border-b border-slate-200 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h3 className="text-sm font-bold text-slate-900">{partner?.username}</h3>
            <p className="text-xs text-slate-400 font-medium">{partner?.status === 'online' ? 'online' : 'last seen recently'}</p>
          </div>
        </div>
        <div className="flex items-center gap-4 text-slate-400">
          <button className="hover:text-blue-500 transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button>
      ...
      <div className="bg-white p-4 border-t border-slate-200">
        <form onSubmit={handleSendMessage} className="max-w-4xl mx-auto flex items-end gap-2">
          <button type="button" className="p-2.5 text-slate-400 hover:text-blue-500 transition-colors">
